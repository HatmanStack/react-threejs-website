...
import './bear.css';
  ...
  const armLimit = Math.random(0, 3);
  const headLimit = Math.random(armLimit + 1, armLimit + 3);
  const angerLimit = Math.random(headLimit + 1, headLimit + 3);
  ...
  export function Bear() {
    ...
    useEffect(() => {
      const grabBearTL = () => {
         ...
         *
         * Paw will go gsap.to scaleX 0.8
         * Arm scaleX goes down gsap.to 0.7
         * Arm wrap translates gsap.to 50% or 50px
         * Bear goes 100% -> 40% -> 0
         */
        let bearTranslation;
        ...
        gsap.to(armRef.current, { scaleX: 0.7, duration: armDuration }).
        ...
        gsap.to(pawRef.current, { duration: pawDuration, scaleX: 0 }, delay).
        gsap.to(
        armRef.current,
      ...
      const showTimeline = () => {
        ...
        gsap.to(indicatorRef.current, { duration: checkboxDuration, x: '100%' }, 0).
